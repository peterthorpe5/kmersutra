...
sample_id="${sample_id/kmersutra_v014/kmersutra_v017}"
sample_id="${sample_id/kmersutra_v015/kmersutra_v017}"
...
work_root="${job_tmp_parent}/kmersutra_v017_${JOB_ID:-manual}_${SGE_TASK_ID}"
...
log "Starting KmerSutra v0.17 lineage-aware screen task ${SGE_TASK_ID}"
...
if [[ -n "${MIN_MIXED_SPECIES_FRACTION}" ]]; then
    command+=(--min_mixed_species_fraction "${MIN_MIXED_SPECIES_FRACTION}")
fi
...
printf '%s\t%s\n' "field" "value" > "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "sample_id" "${sample_id}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "benchmark_family" "${benchmark_family}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "panel" "${panel_label}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "replicate" "${replicate}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "spike_reads" "${spike_reads}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "input_fastq" "${input_fastq}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "source_run_dir" "${source_run_dir}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "source_relative_dir" "${source_relative_dir}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "call_preset" "${CALL_PRESET}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "low_evidence_call" "${LOW_EVIDENCE_CALL}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "min_unique_kmer_margin" "${MIN_UNIQUE_KMER_MARGIN}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "min_unique_kmer_ratio" "${MIN_UNIQUE_KMER_RATIO}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
printf '%s\t%s\n' "min_mixed_species_fraction" "${MIN_MIXED_SPECIES_FRACTION:-preset_default}" >> "${sample_work_dir}/kmersutra_v017_screen_metadata.tsv"
...
printf 'kmersutra_screen_v017_lineage_aware\t%s\t%s\t%s\t%s\n' \
...
log "Finished KmerSutra v0.17 lineage-aware screen for ${sample_id}; runtime=${runtime_seconds}s"
