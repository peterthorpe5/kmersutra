...
RUN_TAG="ai_external_zymo_validation_v050_safe_transformed_$(date +%Y%m%d_%H%M%S)"
...
if [ -z "${MODEL_JSON}" ]; then
    MODEL_JSON="$(find "${BENCH}/ai_validation" -type f \
        -path "*/runs_ai_full_internal_plasmodium_validation_v050_*/outputs/final_internal_calibrator_all_training.json" \
        | sort | tail -n 1)"
fi
