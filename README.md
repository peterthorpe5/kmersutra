...
## v0.35 benchmark post-processing

KmerSutra v0.35 adds a downstream benchmark post-processing command. This step
is intended to be run after database building, sample screening and comparable
benchmark summarisation. It does not build the database and does not prepare AI
training data directly from the reference panel. Instead, it uses the long
call/evidence table generated by screening and summary steps.

The command writes an LCA summary, an AI-ready training table with LCA-derived
numeric features, and a combined HTML/XLSX report:
...
To train and evaluate the interpretable AI call calibrator in the same
post-processing run, add `--train_calibrator`:
...
- `kmersutra_lca_summary.tsv.gz`
- `ai_call_training.tsv.gz`
- `kmersutra_benchmark_postprocess.html`
- `kmersutra_benchmark_postprocess.xlsx`
- `kmersutra_call_calibrator.json` when `--train_calibrator` is used
- `kmersutra_call_calibrator_training_summary.tsv.gz` when `--train_calibrator` is used
- `kmersutra_call_calibrator_evaluation.tsv.gz` when `--train_calibrator` is used
