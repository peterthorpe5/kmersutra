...
def run_summary(
    *,
    ...
    if calls_long.empty:
        missing_preview = status_df.loc[
            ~status_df["calls_exists"].astype(bool),
            [
                "sample_id",
                "benchmark_family",
                "source_relative_dir",
        ...
        LOGGER.error(
            "No detection calls were read. This is not a valid benchmark "
            "summary. First missing paths:\n%s",
            missing_preview,
        )
        raise RuntimeError(
            "No detection calls were read from sample outputs. Check the "
            "manifest sample IDs, source_relative_dir values and output "
            "directory layout under samples/<benchmark_family>/."
        )
