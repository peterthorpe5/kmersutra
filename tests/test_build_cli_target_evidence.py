...
class TestBuildCliAllCandidateEvidence(unittest.TestCase):
    """Tests for all-candidate evidence CLI output files."""

    def test_build_cli_writes_all_candidate_outputs(self) -> None:
        """CLI should produce all-candidate panel and SQLite outputs."""
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            target = root / "target.fna"
            neighbour = root / "neighbour.fna"
            ...
            config.write_text(
                ...
                f"{target}\tAlpha target\tstrain1\t11\ttarget_species\tAlphaGenus\n"
                f"{neighbour}\tAlpha neighbour\tstrain2\t12\tnear_neighbour\tAlphaGenus\n"
                f"{outgroup}\tBeta outgroup\tstrain3\t21\toutgroup\tBetaGenus\n",
                encoding="utf-8",
            )
            out_dir = root / "out"
            command = [
            ...
            if completed.returncode != 0:
                raise AssertionError(completed.stderr)
            self.assertTrue((out_dir / "species_kmer_panel.tsv.gz").is_file())
            self.assertTrue((out_dir / "all_candidate_evidence.sqlite").is_file())
            self.assertTrue((out_dir / "target_evidence_build_summary.tsv").is_file())
            self.assertTrue((out_dir / "build_profile_timing.tsv").is_file())
            self.assertTrue((out_dir / "ram_usage.tsv").is_file())


class TestBuildCliGlobalCandidateEvidence(unittest.TestCase):
    """Tests for scalable global candidate evidence CLI output files."""

    def test_build_cli_writes_global_candidate_outputs(self) -> None:
        """CLI should produce global candidate panel and SQLite outputs."""
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
